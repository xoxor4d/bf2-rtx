...
workspace "bf2-rtx"
	...
	project "bf2rtx"
		...
		if(os.getenv("BF2_ROOT")) then
			print ("Setup paths using environment variable 'BF2_ROOT' :: '" .. os.getenv("BF2_ROOT") .. "'")
			targetdir(os.getenv("BF2_ROOT"))
			debugdir (os.getenv("BF2_ROOT"))
			debugcommand (os.getenv("BF2_ROOT") .. "/" .. "BF2.exe")
		end
		...
		postbuildcommands {
			"echo ----------------------------",
			"echo Rename \"bf2rtx.dll\" to \"bf2rtx.asi\"",
			"MOVE /Y \"$(TargetDir)bf2rtx.dll\" \"$(TargetDir)bf2rtx.asi\"",
		}
